...
inline ship_data_model::Vec3<Real> to_data_model_vec3(const Vector3<Real>& v) {
    ship_data_model::Vec3<Real> out;
    out.x = v.x;
    out.y = v.y;
    out.z = v.z;
    return out;
...
template<typename Real>
inline ship_data_model::Quat<Real> to_data_model_quat(const Quaternion<Real>& q) {
    ship_data_model::Quat<Real> out;
    out.w = q.w;
    out.x = q.x;
    out.y = q.y;
    out.z = q.z;
...
template<typename Real>
inline ship_data_model::Vec3<Real> accel_mps2_to_g(const Vector3<Real>& v) {
...
template<typename Real>
inline ship_data_model::Vec3<Real> gyro_rps_to_deg_s(const Vector3<Real>& v) {
...
public:
    explicit BoatImuDataModelWriter(ship_data_model::DataModel<Real>& model)
        : model_(&model) {}
    ...
    bool write(const BoatImuState<Real>& state) {
        ...
        bool wrote = false;
        ship_data_model::ShipImuData<Real>& imu = model_->imu;
